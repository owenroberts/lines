...
class UI extends ColliderSprite {
	constructor(params, debug) {
		...
		let x = params.x;
		let y = params.y;
		if (x % 1 !== 0) x = GAME.view.width * x; /* decimal / percentage of window */
		if (x < 0) x = GAME.view.width + x; /* negative x offset from right side */
		if (y % 1 != 0) y = GAME.view.height * y; /* decimal / percentage of window */
		if (y < 0) y = GAME.view.height + y; /* negative y offset from bottom */
