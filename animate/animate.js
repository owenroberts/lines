...
function drawEnd(event) {
	...
	if (lines.length > 0)
		if (!lines[lines.length-1].e) lines.pop(); // remove lines with s and no e
	moves = 0;
}
...
function saveFramesToFile(ev) {
	...
	json.d = [];
	var drawingsIndexes = [];
	/* didn't work start here 
	and dont forget click error with drawEnd */
	if (ev.shiftKey) {
		/* save current frame */
		json.f.push( frames[currentFrame] );
		for (var j = 0; j < frames[currentFrame].length; j++) {
			if ( drawingsIndexes.indexOf(frames[currentFrame][j].d) == -1 ) 
				drawingsIndexes.push( frames[currentFrame][j].d );
		}
	} else {
		/* save fall frames */
		json.f = frames;
