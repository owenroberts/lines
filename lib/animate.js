...
function updateFrames() {
	...
	if (frames.length > framenum) {
		for (let i = framenum; i < frames.length; i++) {
			...
			framesPanel.insertBefore(f, plusFrame);
		}
	} else {
		...
		for (let i = framenum; i > frames.length; i--){
			...
			if ( removeFrame.className.includes("current") ) plusFrame.className += " current";
			removeFrame.remove();
		}
	}
	updateFrameNum();
...
function updateFrameNum() {
  	...
  	currentElem.className = currentElem.className.replace(" current", "");
  	const newElem = document.querySelector('#fr'+currentFrame);
  	if (newElem != null) newElem.className = newElem.className += " current";
  	else plusFrame.className = plusFrame.className += " current";
};
