/* convert 2.2 to 2.3 - layers - no frames */
const fs = require('fs');
const file = process.argv[2];
const data = fs.readFileSync(file);
const inn = JSON.parse(data);
const out = {
    v: "2.3",
    w: inn.w,
    h: inn.h,
    fps: inn.fps
};
if (inn.bg) out.bg = inn.bg;
out.mc = inn.mc || false;

/* delete s,e from layers */
out.l = [];
for (let i = 0; i < inn.l.length; i++) {
    out.l[i] = { ...inn.l[i] };
    delete out.l[i].s;
    delete out.l[i].e;
    out.l[i].f = {};
    out.l[i].draw = 'None';
}

/* get first frame, explode status for each layer */
let prev = {};
for (let i = 0; i < inn.f.length; i++) {
    const frames = inn.f[i];
    for (let j = 0; j < frames.length; j++) {
        const frame = frames[j];
        const layer = out.l[frame.l];
        if (i <= layer.f.s || !layer.f.s) layer.f.s = i;
        if (i >= layer.f.e || !layer.f.e) layer.f.e = i;

        if (prev[frame.l]) {
            if (frame.s != undefined && frame.e != undefined) {
                if (prev[frame.l].s != undefined && prev[frame.l].e != undefined) {
                    if (prev[frame.l].e < frame.e) layer.draw = 'Explode';
                    if (prev[frame.l].s < frame.s) layer.draw = 'Reverse';
                }
            }
        }
        prev[frame.l] = { ...frame };
    }
}

/* convert drawing points to arrays */
out.d = [];
for (let i = 0; i < inn.d.length; i++) {
    const drawing = inn.d[i];
    const d = [];
    if (drawing) {
        for (let j = 0; j < drawing.length; j++) {
            const point = drawing[j];
            if (point == "end") d.push(0);
            else d.push([point.x, point.y])
        }
        out.d[i] = d;
    }
}

if (!fs.existsSync('bkup')) {
    fs.mkdirSync('bkup');
}

fs.writeFileSync('bkup/' + file, JSON.stringify(inn));
fs.writeFileSync(file, JSON.stringify(out));
